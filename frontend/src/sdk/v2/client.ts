      ...
      return async () => {
        try {
          ...
          if (
            ...
            if (response.ok) {
              console.warn(
                `[100xprompt] Empty or malformed response body (status ${response.status}, url: ${response.url}). ` +
            ...
            const wrappedError = new Error(
              `Failed to parse ${parseType ?? "response"} from ${response.url} (status ${response.status}): ${e.message}`,
            )
